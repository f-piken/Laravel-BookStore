            ...
            <section class="col-lg-12 col-md-12 shopping-cart">
                <div class="card mb-4 bg-light border-0 section-header">
                    <div class="card-body p-5">
                        <h2 class="mb-0">
                            Order Invoice 
                            <span class="badge {{ $pesan->status == 'pending' ? 'bg-warning' : 'bg-success' }}">
                ...
                <div class="row">
                    <div class="col-lg-7 col-md-6">
                        ...
                        <div class="mt-3">
                            <div class="row">
                                <div class="col-lg-6 col-12 mb-4">
                                    <address>
                                        <strong>{{ $pesan->customer->user->name }}</strong><br>
                                        {{ $pesan->customer->address }}<br>
                                        <abbr title="Phone">P: {{ $pesan->customer ->phone }}</abbr>
                                    </address>
                        ...
                        <div class="mt-3">
                            ...
                            <form action="{{ route('sellers.confirm',$pesan->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="d-flex justify-content-between mt-4">
                                    <a href="{{ route('sellers.pesanan') }}" class="btn btn-second">Back</a>
                                    @if ($pesan->status == 'paid')
                                    <button type="submit" class="btn btn-first">Confirm Order</button>
                                    @elseIf($pesan->status == 'confirm')
                                    <button type="submit" class="btn btn-first">Mark as Process</button>
                                    @endif
                                </div>
                            </form>
                        </div>
